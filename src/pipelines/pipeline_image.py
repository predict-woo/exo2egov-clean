    ...
    def __call__(
        ...
        **kwargs,
        ...
        uncond_clip_image = self.clip_image_processor.preprocess(
            uncond_clip_image, return_tensors="pt",do_rescale=False, do_normalize=False
        ).pixel_values
        # print("uncond_clip_image (line 237): ", uncond_clip_image)
        ...
        # print("uncond_image_prompt_embeds: ", uncond_image_prompt_embeds)

        if do_classifier_free_guidance:
            image_prompt_embeds = torch.cat(
                [uncond_image_prompt_embeds, image_prompt_embeds], dim=0
            )
            # print("image_prompt_embeds (line 247): ", image_prompt_embeds)

        clip_image_exo =  (ref_image + 1) / 2.0
        # print("clip_image_exo(line 247): ", clip_image_exo)
        uncond_clip_image_exo = torch.zeros_like(clip_image_exo)
        # print("uncond_clip_image_exo(line 248): ", uncond_clip_image_exo)
        ...
        # print("clip_image_embeds_exo: ", clip_image_embeds_exo)
        image_prompt_embeds_exo = clip_image_embeds_exo.unsqueeze(1)
        # print("image_prompt_embeds_exo: ", image_prompt_embeds_exo)

        uncond_clip_image_exo = self.clip_image_processor.preprocess(
            uncond_clip_image_exo, return_tensors="pt",do_rescale=False, do_normalize=False
        ).pixel_values
        # print("uncond_clip_image_exo: ", uncond_clip_image_exo)
        uncond_clip_image_embeds_exo = self.image_encoder(
            uncond_clip_image_exo.to(device, dtype=self.image_encoder.dtype)
        ).image_embeds
        # print("uncond_clip_image_embeds_exo: ", uncond_clip_image_embeds_exo)
        uncond_image_prompt_embeds_exo = uncond_clip_image_embeds_exo.unsqueeze(1)        
        # print("uncond_image_prompt_embeds_exo: ", uncond_image_prompt_embeds_exo)
        if do_classifier_free_guidance:
        ...
        # print("latents (line 299): ", latents)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # print("latents (line 342): ", latents)
                ...
                # print("latent_model_input (line 357): ", latent_model_input)
                # print("t (line 358): ", t)
                # print("encoder_hidden_states: ", image_prompt_embeds)
                # print("pose_cond_fea: ", None)
                # print("return_dict: ", False)
                ...
                # print("image_prompt_embeds (line 372): ", image_prompt_embeds)
                # print("noise_pred (line 364): ", noise_pred)

                # perform guidance
                if do_classifier_free_guidance:
                    noise_pred_uncond, noise_pred_text = noise_pred.chunk(2)
                    # print("noise_pred_uncond (line 369): ", noise_pred_uncond)
                    # print("noise_pred_text (line 370): ", noise_pred_text)
                    noise_pred = noise_pred_uncond + guidance_scale * (
                        noise_pred_text - noise_pred_uncond
                    )
                    # print("noise_pred (line 368): ", noise_pred)

                # print("noise_pred: ", noise_pred)
                # print("latents (line 372): ", latents)
                # compute the previous noisy sample x_t -> x_t-1
                latents = self.scheduler.step(
                    noise_pred, t, latents, **extra_step_kwargs, return_dict=False
                )[0]
                # print("latents (line 377): ", latents)
                ...
                # print("latents (line 387): ", latents)
