...
class PixelNeRFTrainer():
    def __init__(self,net,conf,renderer=None,render_par=None,args=None,device='cpu'):
        ...
        self.use_bbox = False
        # Only load fisheye unprojection map if using fisheye camera
        if args.use_fishereye:
            unproj_path = os.path.join(args.pose_dir, "unproj_ego.pt")
            if os.path.exists(unproj_path):
                self.unproj = torch.load(unproj_path)
            else:
                raise FileNotFoundError(f"use_fishereye=True but {unproj_path} not found")
        else:
            self.unproj = None  # Not needed for pinhole cameras
    ...
    def calc_losses(self, data, is_train=True, global_step=0, weight_dtype=torch.float32):
        ...
        for obj_idx in range(SB):
            ...
            # print("ego_images.shape", ego_images.shape)
            c = None
            if "c" in data:
                c = data["c"][obj_idx]
            ...
            # print("image_ego.shape", image_ego.shape)
            ...
            # For fisheye, use pre-computed unprojection map; for pinhole, it's computed from focal/c
            ego_unproj = self.unproj[all_scenes[obj_idx]].to(self.device).to(weight_dtype) if self.args.use_fishereye else None
            cam_rays_ego = util.gen_rays(poses[-1:], data['ego_img'].shape[-2], data['ego_img'].shape[-1], ego_focal, self.ego_z_near, self.ego_z_far, c=ego_c, use_fishereye=self.args.use_fishereye, unproj=ego_unproj)  # (NV, H, W, 8)
            rgb_gt_all = images_0to1
            rgb_gt_all = (
                rgb_gt_all.permute(0, 2, 3, 1).contiguous().reshape(-1, 3)
            )  # (NV, H, W, 3)
            rgb_gt_ego_all = image_ego
            # print("rgb_gt_ego_all.shape", rgb_gt_ego_all.shape)
            rgb_gt_ego_all = (
            ...
            if all_bboxes is not None:
                ...
                if not self.args.use_fishereye:
                    if not self.args.render_whole:
                        ...
                        # render_whole=True: only ego rays for full image rendering
                        pix_inds = None
                        pix_inds_ego = torch.randint(0, data['ego_img'].shape[-2]*data['ego_img'].shape[-1], (self.args.ray_batch_size,))
                else:
                    if not self.args.render_whole:
            ...
            if not self.args.render_whole:
                ...
                # print("not self.args.render_whole rgb_gt.shape", rgb_gt.shape)
                ...
                if pix_inds is not None:
                    ...
                    # print("pix_inds is not None rgb_gt.shape", rgb_gt.shape)
                    all_rgb_gt.append(rgb_gt)
                    all_rays.append(rays)
                if pix_inds_ego is not None:
                    ...
                    rays = rays_ego
                    rgb_gt = rgb_gt_ego
                    # print("pix_inds_ego is not None rgb_gt.shape", rgb_gt.shape)
                    all_rgb_gt.append(rgb_gt)
                    all_rays.append(rays)
        
        # print("all_rgb_gt", all_rgb_gt)
        # for x in all_rgb_gt:
